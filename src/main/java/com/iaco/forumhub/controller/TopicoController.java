...
public class TopicoController {
    ...
    public ResponseEntity criarNovoTopico(@RequestBody @Valid DadosNovoTopico dadosNovoTopico,
        ...
        var topico = new Topico(null, dadosNovoTopico.titulo(), dadosNovoTopico.mensagem(), LocalDateTime.now(),
                Status.OPEN, usuario, curso, null, true);
    ...
    public ResponseEntity getAllTopicos(
            @PageableDefault(size = 2, sort = { "dataCriacao" }, direction = Sort.Direction.DESC) Pageable paginacao,
    ...
    public ResponseEntity updateTopic(@PathVariable Long id,
            @RequestBody @Valid DadosAtualizarTopico dadosAtualizarTopico,
            Authentication authentication) {
        var dadosTopicoAtualizado = topicoService.getDadosDetalhamentoTopicoAtualizado(id, dadosAtualizarTopico,
                authentication);
        return ResponseEntity.ok(dadosTopicoAtualizado);
    }

    // Optei foi fazer a exclusao logica, na sequencia vem o codigo comentado para
    // excluir do DB
    @SuppressWarnings("rawtypes")
    @DeleteMapping("/{id}")
    @Transactional
